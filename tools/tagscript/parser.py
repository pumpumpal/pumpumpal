...
class Parser:
    ...
    def get_tag(self, name, *, parent=None):
        ...
        return next(
            (tag for tag in parent.tags if tag.name == name or name in tag.aliases),
            None,
        )
    ...
    def _base_argument_conversion(self, arg, converter):
        ...
        if converter is bool:
            ...
            if lowered in {"on", "yes", "true", "enable"}:
                return True
            if lowered in {"off", "no", "none", "null", "false", "disable"}:
                return False
            return None
    ...
    def parse_single_tag(self, tag) -> ParsedTag | None:
        """
        ...
        tag_, args = (splitted[0], "") if len(splitted) < 2 else splitted[:2]
        ...
        if args.strip() != "":
            for i, argument in enumerate(arguments):
                ...
                if str(kind) == "VAR_POSITIONAL":
                    buf = [
                        self.do_argument_conversion(then_arg, converter)
