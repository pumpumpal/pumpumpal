    ...
    async def predicate(ctx: commands.Context):
        ...
        if booster:
            ...
            if not user or not user.premium_since:
                raise commands.CommandError(
                    f"You must **boost** the pumpumpal [**Discord Server**](https://discord.gg/pFK6erfxCp) to use `{ctx.command.qualified_name}`"
                )
        ...
        if not donator_check:
            raise commands.CommandError(
                f"You must be a **donator** to use `{ctx.command.qualified_name}` - [**Discord Server**](https://discord.gg/pFK6erfxCp)"
            )
